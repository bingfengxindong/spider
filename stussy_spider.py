...
for url in urls:
    ...
    for goods_url in goods_urls:
        if goods_url.split("?")[1].split("=")[0] == "color_hat":
            ...
            # print(goods_images[0:len(goods_models)*2])
            # g_im = len(g_images)//len(goods_model)
            # goods_images = []
            # for g_i in range(len(goods_model)):
            #     goods_images.append(g_images[g_i*g_im:g_i*g_im+g_im])
            print(goods_images)
